...
% Variable for storing average excitatory synaptic weight values
 ...
 averageInhibitorySynapticWeight = zeros(1,size(time,2));
...
for timestep = 1:size(time,2);
    % Average excitatory synaptic weight value
     ...
     averageInhibitorySynapticWeight(1,timestep) = sum(sum(Weights((4/5)*neuronCount+1:neuronCount,:)),2)/((1/5)*neuronCount*(neuronCount/10));
    ...
    for neuronID = 1:((4/5)*neuronCount) %only inhibitory anti-STDP!
        if(spikes(neuronID, timestep)==1)
            %downStream Neurons
            for j = 1:size(Fanout{neuronID},2)
                downStreamNeuronID = Fanout{neuronID}(j);
                Weights(neuronID,downStreamNeuronID) = Weights(neuronID,downStreamNeuronID)*(1 + Aup*exp(-(timestep*dt+Delay{neuronID}(j)-inSpikes{downStreamNeuronID}(1,size(inSpikes{downStreamNeuronID},2)))/tauL));
            end
        end
    end
            
     for neuronID = (((4/5)*neuronCount)+1):neuronCount %only inhibitory anti-STDP!
        if(spikes(neuronID, timestep)==1)
            %downStream Neurons
            for j = 1:size(Fanout{neuronID},2)
                downStreamNeuronID = Fanout{neuronID}(j);
