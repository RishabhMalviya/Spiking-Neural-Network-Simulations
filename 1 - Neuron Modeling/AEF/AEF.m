function membranePotential = AEF (neuronType, Iapp, dt)
    ...
    V = zeros(500/(dt*1000),1); V(1) = fixedPoint(1e-6, DelT, gL, a, EL, VT);
    U = zeros(500/(dt*1000),1); U(1) = a*(V(1)-EL);
    
    % Perform simulation
    for i = 2:size(V,1)
        % Determine V(n+1) from V(n) using 4th order Runge-Kutta
        [V(i), U(i)] = eulerMethod(V(i-1), U(i-1), dt, Iapp, C, gL, EL, VT, DelT, a, tw);
